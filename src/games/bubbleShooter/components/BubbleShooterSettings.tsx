...
interface BubbleShooterSettingsProps {
...
export const BubbleShooterSettings: React.FC<BubbleShooterSettingsProps> = ({
  isAudioMuted,
  onToggleAudio,
  onResetProgress,
  onClose,
  ...
  const [hapticsEnabled, setHapticsEnabled] = useState<boolean>(() => {
    try {
      return localStorage.getItem('teleplay_bubble_haptics') !== 'false';
    } catch {
      return true;
    }
  });
  ...
  const toggleHaptics = () => {
    ...
    try {
      localStorage.setItem('teleplay_bubble_haptics', next.toString());
  ...
  return (
    <div
      id="bubble-shooter-settings-view"
        ...
        {onResetProgress && (
          <div className="pt-4">
            <button
              type="button"
              onClick={() => {
                if (window.confirm('Reset all Bubble Shooter career progression and scores? This cannot be undone.')) {
                  onResetProgress();
                }
              }}
