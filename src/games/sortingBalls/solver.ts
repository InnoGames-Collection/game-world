...
export function solveBallSortPuzzle(
  ...
  const queue: { state: BallColorKey[][]; moves: number }[] = [
  ...
  while (queue.length > 0 && explored < maxExplored) {
    ...
    for (let i = 0; i < tubes.length; i++) {
      ...
      // Tube already solved and monochrome
      if (
      ...
      const movingColor = tubes[i][tubes[i].length - 1];

      // Measure contiguous group of matching color on top of tube i
      let groupCount = 0;
      for (let k = tubes[i].length - 1; k >= 0; k--) {
        if (tubes[i][k] === movingColor) groupCount++;
        else break;
      }
      ...
      for (let j = 0; j < tubes.length; j++) {
        ...
        if (isDestEmpty) {
          ...
          // Moving an entire monochrome tube to an empty tube is redundant
          if (tubes[i].length === groupCount) continue;
        } else if (tubes[j][tubes[j].length - 1] !== movingColor) {
          continue;
        }
...
export function findNextBestMove(
  ...
  while (queue.length > 0 && explored < maxExplored) {
    ...
    for (let i = 0; i < tubes.length; i++) {
      if (tubes[i].length === 0) continue;
      if (tubes[i].length === capacity && tubes[i].every((c) => c === tubes[i][0])) continue;
      ...
      for (let j = 0; j < tubes.length; j++) {
        ...
        const countToMove = Math.min(groupCount, capacity - tubes[j].length);
        const thisMove = current.firstMove || { from: i, to: j };

        if (!fallbackMove) {
          fallbackMove = thisMove;
        }
        ...
        if (!visited.has(serialized)) {
          visited.add(serialized);
          queue.push({ state: nextTubes, firstMove: thisMove });
        }
      }
    }
  }
